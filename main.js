...
//screen draw
function setupCanvas(barChartData,salaryClean){
    let metric = '0';
    
    function click(){
        metric = this.dataset.name;
        const thisData = chooseData(metric,salaryClean);
        ...
        const dataArraysort = dataArray.sort((a,b)=>b.salary_in_usd - a.salary_in_usd);
    ...
    function update(data){
        ...
        //Update Header
        header.select('tspan').text(` ${metric} % remote ratio data scientist salary ${metric ==='popularity' ? '' : 'in $US'}`);
    ...
    const chart_margin = {top:80,right:80,bottom:5,left:180};
    ...
    header.append('tspan').text('Years:2020-2023')
            .attr('x',0).attr('y',15).style('font-size','0.8em').style('fill','#555');
    ...
    function mouseover(e){
        ...
        // console.log(thisBarData.salary_in_usd);
        const bodyData = [
        ...
        tip.select('h3').html(`${thisBarData.countryname}`);
...
function ready(ds_salaries){
    const salaryClean = chooseData("salary_in_usd",ds_salaries);
    setupCanvas(salaryClean,ds_salaries);
}

function chooseData(metric,salaryClean){
